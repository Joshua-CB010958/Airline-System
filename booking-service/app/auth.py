...
# All services in the cluster share this key so they can each verify tokens
# without delegating to the auth-service on every request.
SECRET_KEY = "airline-jwt-secret-key-change-in-production"
ALGORITHM = "HS256"

# ── OAuth2 Bearer Scheme ───────────────────────────────────────────────────────
# tokenUrl points at auth-service so Swagger UI's "Authorize" button issues
# tokens from the correct service. The Bearer token is then sent to THIS service.
...
def require_roles(allowed_roles: List[str]):
    ...
    Usage:
        ...
        def delete_booking(user = Depends(require_roles(["staff", "admin"]))):
            ...
    """
